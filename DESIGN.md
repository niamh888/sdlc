...
1. **Start screen** — `.page-header` (heading + intro sentence) above a two-column `.quiz-layout`: a narrower checklist column (question count, timer, feedback, ordering, pass mark — the detail behind the intro sentence) beside a wider action card (level notice, name field, Begin button). This mirrors `.contact-layout`'s info-beside-form split rather than inventing a second pattern — see Contact below. The checklist used to open the card itself with no heading of its own, and the card was centred at a fixed width with most of a wide screen empty beside it; both problems are the same root cause, a page built for a much narrower container than the site actually uses. `.quiz-layout` collapses to one column at the same 768px breakpoint `.contact-layout` does.
...
- **No cookie banner.** The site sets no cookies, and all five `localStorage` values (see [README.md](README.md#data-protection) for the full list) exist solely to honour a preference or remember progress the user has asked the site to track, which is exempt as strictly necessary. Adding a consent banner would be theatre.
