...
project's own IEC 62304-style lifecycle *documentation* (development plan,
requirements, architecture, verification, risk management, problem
resolution and the rest, applied reflexively to this project's own
development), see [docs/README.md](docs/README.md).
...
- **The review box's heading matches its state.** `<h3 id="review-form-heading">` reads "Leave a review" only while there is genuinely something to leave; `reviews.js`'s `refreshReviewFormState()` rewrites it to "Your review" the moment a signed-in visitor already has one on file. It used to be static text sitting directly above a status line that could say the review had already been submitted or approved — the heading and the sentence below it contradicted each other. There is no "leave another one" state to design for: one review per person per course is a database-level constraint (`uq_user_course_review` in `backend/app/models.py`), matching how most review platforms work (Amazon, Trustpilot, Udemy, the app stores) — a single review that reflects your current opinion, not a running log.

  **Editing an existing review is future work, not built.** Those same professional sites typically let you edit your review after the fact rather than only ever showing its status; this backend has no route to do that at all — `POST /reviews` is create-only and returns 409 on a second attempt (`backend/app/routers/reviews.py`). Adding it is a real feature, not a copy fix: it needs a new endpoint, and a decision about whether an edit resets `status` back to `pending` for re-moderation (almost certainly yes, since an edited comment is unreviewed content) or is allowed to slide through unmoderated (almost certainly not).
...
5. **Progress tracker** — each card has a "Mark as Studied" button. Clicking it marks the card with a green left border, fills the book icon and the button navy (the same colour, deliberately — see below), and updates the progress bar. Progress persists in `localStorage` (`62304_studiedPhases`, shared across tabs), not just for the session — an earlier in-memory design and then a `sessionStorage` design were both tried and dropped because neither survived a reload or a second tab. The card's footer (Read More / Mark as Studied) is pinned to the card's bottom edge with `margin-top: auto` on a flex column, so a short summary doesn't leave the button floating mid-card while a longer sibling in the same grid row fills its full height; before this fix, the button's vertical position visibly varied card to card. A single "✓ Studied" state lives on the button itself — an earlier version also rendered a separate `.studied-badge` span with the same text next to it, which duplicated the message rather than reinforcing it.
...
**Layout — five screens (shown/hidden with `.active` class), the first two gating the rest:**
0. **Checking session (`#quiz-checking-session`)** — the genuine default on page load: a spinner and "Checking your sign-in status…", the same `.data-status` pattern the Learn page uses while fetching. This exists to fix a real bug, not for symmetry — see below.
1. **Sign-in gate (`#quiz-auth-required`)** — shown instead of everything below once we know nobody is signed in. A quiz result is tied to a real account (see [Backend Architecture](#backend-architecture)), so the assessment cannot even be *started* while signed out, not just left unable to save at the end — a heading, one sentence explaining why, and a single "Log In / Sign Up" button.
2. **Start screen** — `.page-header` (heading + intro sentence) above a two-column `.quiz-layout`: a narrower checklist column (question count, timer, feedback, ordering, pass mark — the detail behind the intro sentence) beside a wider action card (level notice, a name element, Begin button). This mirrors `.contact-layout`'s info-beside-form split rather than inventing a second pattern — see Contact below. The checklist used to open the card itself with no heading of its own, and the card was centred at a fixed width with most of a wide screen empty beside it; both problems are the same root cause, a page built for a much narrower container than the site actually uses. `.quiz-layout` collapses to one column at the same 768px breakpoint `.contact-layout` does.

   **The "name element" is one of two things, not always a field to fill in.** An account created through `login.html`'s sign-up form already has a full name on it, so the normal case is `#quiz-welcome` — a plain "Welcome, `<name>`!" line, nothing to type. `#quiz-name-group`'s text input (`#participant-name`) is a fallback shown only when the signed-in account has no stored name (an account created before this field existed), and `quiz.js` hides whichever of the two it isn't using. This replaced an earlier design where every learner, signed in or not, retyped their name on every attempt — once results were tied to a real account, the account's own name became the more truthful default, and asking again read as not trusting data the site already had.
3. **Question screen** — progress bar, countdown timer, question text, four answer buttons, feedback panel
4. **Results screen** — score, pass/fail heading, breakdown (correct / incorrect / percentage), retry and review buttons

**The checking-session screen fixes a reported bug, not a style preference.** `quiz.js` decides which of the gate/start screens applies by asking the backend to verify the stored token (`getCurrentSession()` → `GET /auth/me`) — a real network round trip, not an instant local check, and one that reliably took around a second. Screens 1 and 2 used to be the only two states, with screen 2 (the start screen) marked `active` in the static HTML because it's the common case. That meant every visitor — signed in or not — was shown the start screen, fallback name field and all, for however long that round trip took, *then* watched it get silently rewritten once the real answer arrived: to the sign-in gate if they turned out to be signed out, or to "Welcome, `<name>`!" if signed in. For a returning signed-in learner this looked exactly like the pre-account version of the site asking them to type their name again — reported directly as confusing (the field disappearing gives no warning that it's about to, so someone unfamiliar with the site could easily have started typing into it before it vanished). The fix is the same principle the Learn page already applies to its own async load: never show a screen that might be wrong, show an honest "finding out" state instead, and reveal the real one — fully populated — only once it's actually known. `quiz.js`'s `DOMContentLoaded` handler now calls `showQuizScreen('quiz-start')` itself, at the end of a synchronous block that has already set the welcome/name-group state and the level notice, rather than relying on the start screen being pre-marked active in markup.
...
**Asynchronous behaviour:** The question file is *prefetched* — the request starts on page load while the learner is still reading the instructions, and the resulting Promise is stored in `questionsPromise`. `startQuiz()` awaits that stored Promise on click, so the data is normally already there and the click feels instant; a learner who clicks immediately sees the button switch to "Loading questions…" and joins the wait already in progress. When the fallback name field is in play (see above), its validation runs before any awaiting, so an empty name is rejected instantly with no network activity — an account with a stored name has nothing to validate at this step at all, since there is no field to leave empty. The Begin button is disabled during the load to prevent a double-click starting two quizzes at once, and restored in a `finally` block so it can never be left stuck.
...
**Purpose:** Dual-purpose, matching the project brief's own dual purpose — establishes why the course's content can be trusted (the author's standing credentials) and doubles as a marketing page for the consultancy behind it, St John Lynch & Co.

**Layout:**
- `.page-header` with just the `<h1>` — no intro paragraph, unlike every other page's header
- `.about-intro` — one lede paragraph naming the author (St John Lynch & Co Ltd) and its domain (SaMD, SiMD, Health Software, AI-enabled Health Software)
- Two `.qual-grid` sections, each three `.qual-card`s:
  - **Technical qualifications** — ISO 13485 (quality management), ISO 27001 (information security), IEC 81001-5-1 (health software security)
  - **Experiential qualifications** — hands-on development (not just auditing), regulatory strategy through to market submission, and audits actually conducted against the three standards above
- A short "this site practices what it teaches" section — the same reflexive-lifecycle idea `docs/` demonstrates at the artefact level (see [docs/README.md](docs/README.md)), stated here in prose
- `.about-cta` — links out to stjohnlynch.com and in to the Contact page

**Key decision — `.about-lede` is 960px, not the site's usual 760px reading measure.** Every other block of long-form prose (`.legal-page`, `.about-intro`, `.about-cta`) caps at 760px for readability (see Responsiveness below). `.about-lede` is the one exception, because each instance sits directly above a full-width `.qual-grid` — at 760px the lede visibly stopped short of the grid's edge and read as a layout mistake rather than a design choice.

**Deferred:** a dedicated trust-signals/services section — named pillars (Training / Software Dev / QA-RA) — was discussed and set aside for a future pass rather than built speculatively.
...
**Layout:**
- Two-column grid: contact info panel (left) + form (right), collapses to single column on mobile — the same info-beside-action idiom the Quiz start screen now uses for its own two-column `.quiz-layout` (see Quiz above)
...
**Key decisions:**
- **Plain-language summary first.** A highlighted box at the top states the handful of things that matter most — currently eight bullets, covering both the contact form and the account/quiz/review data below — so nobody has to read the full page to learn what happens to their data. A privacy notice that is technically complete but practically unread does not achieve transparency.
...
- **The quiz's data handling changed, and the page was rewritten rather than patched.** This page originally stated that a quiz result lives in browser memory only and is never sent anywhere — true when there was no backend to send it to. Once accounts, saved attempts and certificate verification were built (see [Backend Architecture](#backend-architecture)), that sentence became false, so the "Accounts, quiz results and certificates" section replaced it outright rather than being quietly edited around: it says directly that this used to say the opposite, states what is actually collected (email, hashed password, optional name; each attempt's name, score, level and timestamps), and states where it is processed — Neon/Render, both pinned to the EU (Frankfurt), so unlike the contact form this data never leaves the EEA at all. A stale privacy claim is worse than an absent one, since a reader has no reason to doubt it.
- **No cookie banner.** The site sets no cookies for a visitor or learner, and all six `localStorage` values (see [README.md](README.md#data-protection) for the full list) exist solely to honour a preference, remember progress, or keep a signed-in session the user asked for — which is exempt as strictly necessary. The one exception, a login cookie on the separate `/admin` review-moderation tool, is not something a learner ever encounters and is disclosed on the page for completeness. Adding a consent banner for any of this would be theatre.
...
A deep navy and clinical blue palette was chosen to match the professional, regulatory context of the subject matter. The same palette is used in medical device company documentation and audit tools, so it signals credibility to the target audience. The site supports a light and a dark theme (`theme.js`, toggled in the header, persisted to `localStorage`), and every colour lives in a CSS custom property so a theme is a matter of redefining variables rather than restyling components.

**Tokens are split by ROLE, not by hue — the split that makes two themes tractable at all:**

| Role | Behaviour across themes | Examples |
|---|---|---|
| **Fills** — solid backgrounds carrying white text | Stay dark in *both* themes. White-on-dark reads fine on a light page and a dark one; lightening a fill to "suit" dark mode is the trap — the white label on top stops being readable. | `--primary` `#1a3a5c`, `--accent-fill` `#2471a3`, `--success-fill` `#1a7a4a`, `--danger-fill` `#a93226` |
| **Text colours** | *Must* flip between themes — a near-black navy used for headings is invisible on a dark background. | `--heading`, `--accent`, `--success-text`, `--danger-text`, `--text-muted` |
| **Tints** | Pale panel backgrounds (notices, badges, answer feedback) plus the text/border pair that sits on each one. | `--tint-info` / `--tint-info-text`, `--tint-ok` / `--tint-ok-text`, `--tint-warn` / `--tint-warn-text` |

Every foreground/background pairing is checked numerically against WCAG 2.1 AA rather than judged by eye — several of the `--*-text` values in `style.css` carry a comment recording the exact ratio and the earlier value that fell short. Eyeballing a dark palette is unreliable in a way eyeballing a light one is not: light text on a dark field reads as higher-contrast than it measures. The full, current token list lives in `style.css`'s `:root` block (light theme) and its `prefers-color-scheme: dark` / `[data-theme="dark"]` blocks (dark theme) — this table is the shape of the system, not a mirror of every value, because the values change more often than the role split does.
...
Three breakpoints:
- Default: desktop (`.container` max-width **1320px** — widened from an original 1100px once wide-screen testing showed real dead space in the card grids and the two-column layouts below; long-form prose blocks (`.legal-page`, `.about-intro`, `.about-cta`) deliberately stay capped at 760px regardless, since a wider reading measure hurts readability rather than helping it — see `.about-lede`'s 960px exception under About above)
- `max-width: 768px`: tablet — hero collapses to single column; `.contact-layout` and `.quiz-layout` (Quiz's checklist-beside-card grid, see Quiz above) both collapse from two columns to one, the same breakpoint for both since they're the same pattern
- `max-width: 480px`: mobile — hero stats and CTA buttons stack vertically, nav links shrink

Not every reflow is breakpoint-driven. The footer's "Developed by … · Privacy & Data Protection" line uses `flex-wrap` rather than a fixed breakpoint, so it degrades gracefully at whatever width it stops fitting on one line instead of jumping at one fixed number — appropriate for a line of text with no fixed content width, unlike a card grid.
