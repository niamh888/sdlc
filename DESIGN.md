...
6. **Promotional side rails (ultra-wide screens only)** — `.container`'s own 1100px max-width leaves two large empty gutters on any monitor wider than about 1600px; above that width, two sticky cards (St John Lynch & Co; AskRiskIE, a recommended ISO 14971 risk management platform) fill them, each linking out to the respective site. Below the breakpoint `.side-rail` is plain `display:none` — not resized, not restacked, simply absent — so none of the four viewport widths the responsive test suite already covers (1280/768/480/360) changes at all; this is purely additive at widths wider than any of them. `#main-content` becomes a three-column grid only inside that same `@media screen and (min-width: 1600px)` query, with the rail markup placed *after* the main `.container` in the HTML source (not before or between) precisely so a screen reader or keyboard user reaches the actual course content first — explicit `grid-column` placement is what lets the rails still appear visually beside the content despite coming after it in the DOM. The cards use `width:100%` with a `max-width` rather than a fixed pixel width, which is what guarantees they can never force horizontal overflow: a `1fr` grid track is sized by the browser to whatever space is genuinely left over, so the card can only ever end up narrower than its cap. Hidden on print with an explicit rule, independent of and in addition to the `screen and` scoping on the breakpoint query.
