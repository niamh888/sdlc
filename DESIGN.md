...
**Layout — four screens (shown/hidden with `.active` class), the first gating the other three:**
0. **Sign-in gate (`#quiz-auth-required`)** — shown instead of everything below when nobody is signed in. A quiz result is tied to a real account (see [Backend Architecture](#backend-architecture)), so the assessment cannot even be *started* while signed out, not just left unable to save at the end — a heading, one sentence explaining why, and a single "Log In / Sign Up" button. `quiz.js` checks the session once on page load, before anything else runs.
1. **Start screen** — `.page-header` (heading + intro sentence) above a two-column `.quiz-layout`: a narrower checklist column (question count, timer, feedback, ordering, pass mark — the detail behind the intro sentence) beside a wider action card (level notice, a name element, Begin button). This mirrors `.contact-layout`'s info-beside-form split rather than inventing a second pattern — see Contact below. The checklist used to open the card itself with no heading of its own, and the card was centred at a fixed width with most of a wide screen empty beside it; both problems are the same root cause, a page built for a much narrower container than the site actually uses. `.quiz-layout` collapses to one column at the same 768px breakpoint `.contact-layout` does.
