...
**Clause:** 5.4 — Software Detailed Design · **Register:** [Document Register](README.md)
...
**`learn.js` has a real detailed design document: `learn_pseudocode.md`.**
...
file in the project where 5.4.2–5.4.3 are actually satisfied, not just
...
(compare `learn_pseudocode.md` against
`tests/test_site.py`'s "the site refuses to render on
...
comments and the code itself, plus the behavioural description in the
project's design notes. For a real Class C project this would be a finding:
5.4.2 requires detailed design for *each* software unit, not the one that
...
useful document than writing all five today for their own sake.
